        ...
        if (hasHas && object.type === 'IfStatement' && object.test.type &&
            ...
            if (object.type === 'ExpressionStatement' && object.expression &&
                ...
                if (tempObject.params && tempObject.params.length) {
                    ...
                    // Hack(Eric): mixin is sometimes undefined for no apparent reason
                    // I've just copied the mixin functions from lang.js
                    if(!mixin) {
                        var mixin1 = function(dest, source, override){
                            var name;
                            for (name in source) {
                        ...
                        var mixin = function(dest){
                            ...
                            for (i = 1, l = parameters.length; i < l; i++) {
                                mixin1(dest, parameters[i], override);
                            }
                            return dest; // Object
                        };
                    }
